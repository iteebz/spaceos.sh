...
export const spawnSpawnNode = (state: GalaxyState): void => {
  ...
  spawns.push({
    ...
    vy: 0,
    size:
      SPAWN_CONFIG.sizeMin +
      Math.random() * (SPAWN_CONFIG.sizeMax - SPAWN_CONFIG.sizeMin),
    color: AGENT_COLORS[agentIndex],
    life:
      SPAWN_CONFIG.lifetimeMin +
      Math.random() * (SPAWN_CONFIG.lifetimeMax - SPAWN_CONFIG.lifetimeMin),
    maxLife:
      SPAWN_CONFIG.lifetimeMin +
      Math.random() * (SPAWN_CONFIG.lifetimeMax - SPAWN_CONFIG.lifetimeMin),
    phase: "spawning",
