...
export const updateStar = (
  ...
  const armPhase = Math.sin(
    Math.atan2(dy, dx) * PHYSICS.arms + dist * PHYSICS.armTightness - spiralRotation + armNoise
  )
...
export const drawGalaxy = (ctx: CanvasRenderingContext2D, state: GalaxyState, alpha: number) => {
  const { width, height, mouseX, mouseY, gravityActive, time, deltaTime, stars, flashes } = state
...
export const drawFlashes = (ctx: CanvasRenderingContext2D, state: GalaxyState, alpha: number) => {
