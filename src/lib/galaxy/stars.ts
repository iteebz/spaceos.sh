...
export const renderStar = (
  ...
  const opacity =
    ...
    alpha
  const drift = {
    x: Math.sin(time * VISUAL.driftSpeed + star.baseX * VISUAL.driftFreq) * VISUAL.driftScale,
    y: Math.cos(time * VISUAL.driftSpeed + star.baseY * VISUAL.driftFreq) * VISUAL.driftScale,
  }
  ...
  if (speed > VISUAL.trailSpeedMin && dist < VISUAL.trailDist) {
    ...
    ctx.lineWidth = Math.max(VISUAL.trailWidthMin, star.size * VISUAL.trailWidthScale)
    ctx.beginPath()
